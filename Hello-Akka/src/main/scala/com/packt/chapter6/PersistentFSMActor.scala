...
class PersistentFSMActor(_persistenceId: String)(implicit val domainEventClassTag: ClassTag[DomainEvent]) extends PersistentFSM[CountDownLatchState, Count, DomainEvent] with ActorLogging {
...
object PersistentFSMActor {
  def props(persistentId: String) = Props(new PersistentFSMActor(persistentId))
}
