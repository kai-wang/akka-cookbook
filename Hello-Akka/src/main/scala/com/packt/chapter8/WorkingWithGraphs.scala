...
import akka.stream.scaladsl.{Balance, Broadcast,
Flow, GraphDSL, Merge, RunnableGraph, Sink, Source}
...
object WorkingWithGraphsApplication extends App{
  ...
  val graph = RunnableGraph.fromGraph(GraphDSL.create() {implicit builder =>
    ...
    def counter = Flow[Seq[GenericMsg]].via(new
        StatefulCounterFlow())
    val mapper = Flow[Seq[GenericMsg]].mapConcat(_.toList)

    //Junctions
    val aBroadcast =
      builder.add(Broadcast[Seq[GenericMsg]](2))
    val iBroadcast =
      builder.add(Broadcast[Seq[GenericMsg]](2))
    val balancer = builder.add(Balance[Seq[GenericMsg]](2))
    val notitificationMerge =
      builder.add(Merge[Seq[GenericMsg]](2))
    val genericNotitificationMerge =
      builder.add(Merge[GenericMsg](2))

    //Sink
    def counterSink(s: String) = Sink.foreach[Int](x =>
      println(s"$s: [$x]"))
